...
const backer_reducer = (acc, game) => acc + game.backers;
const total_backers = GAMES_JSON.reduce(backer_reducer, 0);
...
const raised_reducer = (acc, game) => acc + game.pledged;
const amount_raised = GAMES_JSON.reduce(raised_reducer, 0);
console.log({amount_raised});
...
const unfund_total = (GAMES_JSON.filter((game) => game.pledged < game.goal)).length;
console.log(unfund_total);
// create a string that explains the number of unfunded games using the ternary operator

const displayStr = `A total of $${amount_raised.toLocaleString("en-US")} has been raised for ${GAMES_JSON.length} games. Currently, ${unfund_total === 1 ? 
    "1 game remains unfunded. We need your help to fund these amazing games!" : 
    unfund_total === 0 ? "no games remain unfunded. Thank you for your support in funding these games!" : 
    `${unfund_total} games remain unfunded. We need your help to fund these amazing games!`}`; 

// create a new DOM element containing the template string and append it to the description container

const fundingStatus = document.createElement("p");
fundingStatus.innerHTML = displayStr;
...
top_pledge.innerHTML = first_game.name;
firstGameContainer.appendChild(top_pledge);
// do the same for the runner up item
const runner_up = document.createElement("p");
runner_up.innerHTML = second_game.name;
